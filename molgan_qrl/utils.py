...
class MolecularMetrics(object):
    ...
    def valid_filter(mols):
        valid_molecules = list(filter(MolecularMetrics.valid_lambda, mols))
        print(f"Total number of valid molecules: {len(valid_molecules)}")
    ...
    @staticmethod
    def water_octanol_partition_coefficient_scores(mols, norm=False): #logP 
